...
- **自动分表** — 通过 `storage_filewords` 配置时间格式（如 `YYYY-MM-DD-hh`），程序自动按时间窗口创建独立数据表，到达新时间窗口时无需重启即自动切换，游标和未完成任务自动迁移到新表
...
| 配置 | 说明 |
|------|------|
| `storage_filewords` | 时间格式模板，如 `YYYY-MM-DD-hh`，程序按当前时间生成表名前缀和媒体目录 |
...
### storage_filewords 说明

该配置决定数据库表名前缀和媒体文件目录名。使用 `YYYY`/`MM`/`DD`/`hh`/`mm`/`ss` 占位符，程序启动时按当前时间替换，**运行中到达新时间窗口时自动切换，无需重启**。
...
| `YYYY-MM-DD-hh-mm` | `2026-04-11-15-30` | `corp_2026-04-11-15-30_messages` | 每分钟 |
...
表名由 `storage_filewords` 配置动态生成，格式为 `corp_{时间前缀}_xxx`。例如 `storage_filewords: "YYYY-MM-DD-hh"` 在 04 点运行时：

| 表 | 说明 |
|----|------|
| `corp_2026-04-11-04_messages` | 所有解密后的消息，`content` 字段为完整 JSON |
| `corp_2026-04-11-04_seq_cursors` | 每个企业的拉取进度（seq 游标） |
| `corp_2026-04-11-04_media_tasks` | 媒体文件下载队列，status: 0=待下载 1=下载中 2=完成 3=失败 |
...
示例：
/opt/wxworkChatData/media/fpxinxi/2026-04-11-04/voice/xxxxx.amr
/opt/wxworkChatData/media/fpxinxi/2026-04-11-04/image/xxxxx.jpg
/opt/wxworkChatData/media/fpxinxi/2026-04-11-04/emotion/xxxxx.png
```
